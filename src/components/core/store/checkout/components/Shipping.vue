...
const methods = computed(() => {
  return shippingMethods.value.filter((method: { name: string; }) => method.name.toLowerCase().indexOf(selectedFilter.value.toLowerCase()) !== -1)
})
    ...
    <template v-if="enableFilter">
      <div class="flex gap-4 items-center">
        <div class="flex items-center gap-2">
          <input type="radio" name="shipping-speed" id="express" value="express" v-model="selectedFilter" />
          <label for="express">Express Delivery</label>
        </div>
        <div class="flex items-center gap-2">
          <input type="radio" name="shipping-speed" id="standard" value="standard" v-model="selectedFilter" />
          <label for="standard">Standard</label>
        </div>
        <div class="flex items-center gap-2">
          <input type="radio" name="shipping-speed" id="pick-up" value="pick-up" v-model="selectedFilter" />
